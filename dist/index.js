...
module.exports = function (app) {
    ...
    const plugin = {
        ...
        start: function (props) {
            const delta = new signalk_libdelta_1.Delta(app, plugin.id);
            ...
            if (positionStream) {
                ...
                unsubscribes.push(positionStream.onValue((position) => {
                    ...
                    if ((options.lastday != today) || (Math.abs(options.lastposition.latitude - position.latitude) > 1.0) || (Math.abs(options.lastposition.longitude - position.longitude) > 1.0)) {
                        ...
                        if ((options.times = (0, suncalc_1.getTimes)(now, position.latitude, position.longitude)) && (options.times !== undefined)) {
...
function parseTimeString(s, sunphases) {
    ...
    if (matches = (s.trim()).match(/^(\d\d):(\d\d):(\d\d)$/)) {
        ...
        if (sunphases.hasOwnProperty(matches[1])) {
            ...
            switch (matches[4]) {
                case 'h':
                    retval += (((matches[2] == "+") ? 1 : -1) * Number(matches[3]) * 3600);
                    break;
                case 'm':
                    retval += (((matches[2] == "+") ? 1 : -1) * Number(matches[3]) * 60);
                    break;
                case 's':
                    retval += (((matches[2] == "+") ? 1 : -1) * Number(matches[3]));
                    break;
            }
        }
        else {
